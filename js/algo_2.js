...
function clustering(clusters, arrayOfPoints) {
    ...
    while (true) {
        for (i = 0; i < arrayOfPoints.length; i++) {
            currentPointCoords = arrayOfPoints[i];
            min = distance(clusters[0][0], currentPointCoords);
            indexMin = 0;
...
window.onload = function () {
    ...
    button.onclick = function () {
        ...
        if (numberOfClusters > arrayOfPoints.length)
            alert("Кластеров больше, чем точек");
        else {
            let clusters = [];
